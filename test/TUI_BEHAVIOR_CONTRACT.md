...
| New/fork/restore append a session boundary without clearing earlier terminal history                     | `tui-controller.test.ts`, `production-test.tsx:testTranscriptWriter`                                      |
| `/clear` starts a fresh session and clears terminal output and saved scrollback once                     | `tui-controller.test.ts`, `opentui-state.test.ts`, `production-test.tsx:testTranscriptWriter`             |
