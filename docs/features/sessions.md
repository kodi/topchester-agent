...
Session events always provide order and mixed event gaps. Exact timing breakdowns require `TOPCHESTER_LOG_LEVEL=debug` or `trace` before the measured work starts. New timing records include session and turn identifiers, so the report can separate concurrent root and child work. Hook records also include privacy-safe handler labels, handler ordinals, configured timeouts, process-exit timing, and final stream-close timing. They do not include the full configured command. Old unscoped log entries are not assigned to a session.
