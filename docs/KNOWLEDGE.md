...
The KB is mutable workspace knowledge under `topchester-kb/` (or `TOPCHESTER_KB_DIR`) and is managed by project KB commands. Automatic retrieval uses that project source when it is ready.

Topchester-specific product help is not another KB source. It lives in packaged static skills so it remains available when the current workspace has no project KB.
...
See `docs/drift-detection.md` for the implementation-oriented drift checker design.
...
Context packs should guide the task, not replace live source inspection. The agent
should use them to orient, plan, find local patterns, estimate impact, and identify
likely verification commands. For task-critical facts, the agent must resolve the
claim against the current working tree before acting.

Context pack entries should be provenance-aware:

```json
{
  "source": "canonical_kb",
  "drift": "current",
  "evidence": ["file:src/server/routes/users.ts"],
  "hash": "sha256:..."
}
```
...
Topchester uses the KB to choose context, plan work, estimate impact, and maintain
continuity, but it must resolve task-critical facts against the current working
...
4. Use KB context to find likely files, modules, features, examples, tests, and commands.
5. Read current source files before relying on task-critical details.
6. If the relevant KB is stale, say so and either refresh it or operate in a clearly degraded mode.
7. During edits, update the session overlay so dirty knowledge is known.
8. After editing code, update or mark affected KB entries stale or suspect.
9. Before commit or PR handoff, either synchronize the canonical KB or leave an explicit `needs_sync` state.
10. Commits should include KB updates whenever in-scope files changed, because every change is potentially semantic.
11. The TUI should make KB state visible: current/stale/missing/suspect.
...
- Session overlay view: known dirty files, suspect modules/features, and sync state.
...
14. Task-time KB policy: use the KB to choose context, plan work, estimate impact, and maintain continuity, but resolve task-critical facts against the current working tree before acting.
15. Session overlay policy: dirty KB state is acceptable during active work if Topchester knows which knowledge is dirty and has a clear synchronization path.
...
3. The session overlay storage model and lifecycle.
4. The escape-hatch workflow for framework behavior that is unclear from local code and metadata.
...
8. Session overlay for dirty-but-known active work, so the agent can combine canonical KB, current working tree, and session changes.
9. Local HTTP JSON-RPC API with:
   - `kb.search`,
   - `kb.getNode`,
   - `kb.neighbors`,
   - `kb.contextPack`,
   - `kb.driftCheck`,
   - `kb.impact`.
10. MCP adapter over the same KB service.
11. Generated SQLite cache for fast search/FTS, ignored by git.
12. TUI views for scan progress, feature map, context pack, session overlay, and drift state.
...
- session overlay: what changed during active work,
- API: fast context assembly, impact analysis, and KB-aware task planning.
